...
/**
 * @brief 将种子文件的内容读入到全局变量metafile_content所指向的缓冲区
 *
 * @param metafile_name 种子文件名
 * @return int          处理成功返回0，否则返回−1
 */
int read_metafile(char *metafile_name) {
    long i;

    // 以二进制、只读的方式打开文件
    // 以字符方式打开可能无法读取整个文件的内容，piece的hash值中可能含有字符0x00
 ...
 * @return int     成功执行并找到关键字返回1，未找到返回0，执行失败返回−1
 */
...
//
/**
 * @brief 添加新的 tracker 服务器 URL
 *
 * @param url
 * @return int url存在，返回0；成功添加，返回1
 */
...
/**
 * @brief 获取每个piece的hash值，并保存到pieces所指向的缓冲区中
 *
 * @return int 成功返回0，读取失败返回-1
 */
int get_pieces() {
...
/**
 * @brief 获取待下载的文件的文件名，如果下载的是多个文件，则获取的是目录名
 *
 * @return int 成功返回0，读取失败返回-1
 */
int get_file_name() {
...
int get_files_length_path() {
    ...
    for (i = 0; i < filesize - 8; i++) {
        if (memcmp(&metafile_content[i], "6:length", 8) == 0) {
            ...
            node->length = length;
            node->next = NULL;
            if (files_head == NULL) files_head = node;
            else {
                p = files_head;
                while (p->next != NULL) p = p->next;
                p->next = node;
...
int get_info_hash() {
    ...
    for (; i < filesize;)
        if (metafile_content[i] == 'd') {
            ...
            (metafile_content[i] >= '0') && (metafile_content[i] <= '9')) {
            int number = 0;
            while ((metafile_content[i] >= '0')
                   && (metafile_content[i] <= '9')) {
    ...
    SHA1Init(&context);
    SHA1Update(
        &context, (unsigned char *)&metafile_content[begin], end - begin + 1);
    SHA1Final(info_hash, &context);

#ifdef DEBUG
    printf("info_hash:");
    for (i = 0; i < 20; i++) printf("%.2x ", info_hash[i]);
    printf("\n");
#endif
