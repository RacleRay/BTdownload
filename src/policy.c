...
float total_down_rate = 0.0F,
      total_up_rate = 0.0F; // 总的下载上传速度, 单位是B/S而不是KB/S
int total_peers = 0;        // 已连接的总Peer数

extern int end_mode;      // 是否已进入终端模式
extern Bitmap *bitmap;    // 指向本地的位图
...
// 假设要下载的文件共有100个piece
// 片段选择时，先将0到99这100个数的顺序以随机的方式打乱
// 从而得到一个随机的数组, 供片断选择算法使用
int *rand_num = NULL;

/**
 * @brief 初始化全局变量unchoke_peers，其存放非阻塞Peer和优化非阻塞Peer的指针
 *
 */
...
/**
 * @brief 判断一个peer是否已经存在于unchoke_peers
 *
 * @param node peer指针
 * @return int 存在返回1，不存在放回0
 */
...
/**
 * @brief 从unchoke_peers中获取下载速度最慢的peer的索引
 *
 * @param array unchoke_peers数组
 * @param len unchoke_peers数组的长度
 * @return int 下载速度最慢的peer的索引
 */
...
/**
 * @brief 找出当前下载速度最快的4个peer，将其unchoke
 *
 * @return int
 */
int select_unchoke_peer() {
    ...
    // NOTE：up_rate和down_rate的单位是B/S而不是KB/S
...
/**
 * @brief 生成一个在总 piece 个数范围内的随机数
 *
 * @param length 要下载的文件的总 piece 个数
 * @return int 执行出错返回-1，执行成功返回0
 */
...
/**
 * @brief 从peer队列中选择一个作为优化非阻塞peer
 *
 * @return int 出错返回-1，否则返回0。注意如果 peer 数目小于4，也会返回0
 */
...
/**
 * @brief 计算最近一段时间(如10秒)每个peer的上传下载速度
 *
 * @return int
 */
int compute_rate() {
    ...
    while (p != NULL) {
        if (p->last_down_timestamp == 0) {
            ...
            if (t == 0) printf("%s:%d time is 0\n", __FILE__, __LINE__);
                ...
                p->down_rate = (double)p->down_count / t;
            p->down_count = 0;
...
/**
 * @brief 生成request请求消息,实现了片断选择算法,17为一个request消息的固定长度
 *
 * @param node
 * @return int
 */
int create_req_slice_msg(Peer *node) {
    ...
    // 然后去btcache_head中寻找满足下面条件的piece:
    // 1. 它没有下载完
    // 2. 它不在任何peer的request消息队列中
    // 应该优先下载这样的piece。
    // 出现这样的piece的原因是:
    // 从一个peer处下载一个piece,还没下载完,那个peer就将我们choke了或下线了

    // 但是测试结果表明, 以上面这种方式这种方式创建rquest请求执行效率并不高
    ...
    while (count < 4) {
        ...
        printf(
            "*** create request index:%-6d begin:%-6x to IP:%s ***\n", index,
            q->begin, node->ip);
        begin += 16 * 1024;
        count++;
    }
...
// /**
//  * @brief Create a req slice msg from btcache object
//  *
//  * @param node
//  * @return int
//  */
...
//     // 指针 b 用于遍历btcache缓冲区
//     // 指针 b_piece_first 指向每个piece第一个slice处
...
//     int count = 0, num;
//     int valid_count; // valid_count指明一个piece中已下载的slice数
...
//                             q->next = NULL;
//                             printf(
//                                 "create request from btcache index:%-6d begin:%-6x\n",
//                                 index, q->begin);
//                             if (r == NULL) {
//                                 node->Request_piece_head = q;
//                                 r = q;
