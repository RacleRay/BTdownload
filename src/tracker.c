...
/**
 * @brief 进行编码转换，根据HTTP协议，HTTP请求中的非数字和非字母都要进行编码转换
 *
 * @param in HTTP请求消息字符串
 * @param len1 HTTP请求消息字符串长度
 * @param out 处理后的消息指针
 * @param len2 处理后的消息长度
...
int http_encode(unsigned char *in, int len1, char *out, int len2) {
    ...
    for (i = 0, j = 0; i < 20; i++, j++) {
        if (isalpha(in[i]) || isdigit(in[i])) out[j] = in[i];
        else {
            out[j] = '%';
            j++;
...
/**
 * @brief 获取Tracker URL中的主机名部分
 *
 * @param node Tracker URL
 * @param name 主机名
 * @param len 主机名字符长度
 * @return int 失败返回-1，成功返回0
 */
...
/**
 * @brief 获取Tracker URL中的端口号
 *
 * @param node Tracker URL
 * @param port 端口号输出位置指针
 * @return int 失败返回-1，成功返回0
 */
...
/**
 * @brief 构造发送到Tracker服务器的HTTP GET请求
 *
 * @param request 用于保存生成的请求消息
 * @param len request所指向的字符数组的长度
 * @param node Tracker URL
 * @param port 监听的端口号
 * @param down 已下载的数据量
 * @param up   已上传的数据量
 * @param left 剩余多少字节未下载
 * @param numwant 希望Tracker返回的Peer数
 * @return int
 */
int create_request(
    char *request,
...
/**
 * @brief 获取Tracker返回的消息的类型
 * 
 * @param buffer Tracker的回应消息的储存位置
 * @param len buffer所指向的字符数组的长度
 * @param total_length Tracker返回数据的长度
 * @return int 
 */
...
/**
 * @brief 以非阻塞的方式连接Tracker
 * 
 * @param max_sockfd sock文件描述符的最大值
 * @return int 
 */
int prepare_connect_tracker(int *max_sockfd) {
    ...
    for (i = 0; i < tracker_count; i++) {
        if (valid[i] != 0) {
            ...
            // 连接tracker
            ret = connect(
                sock[i], (struct sockaddr *)&tracker[i],
...
int prepare_connect_peer(int *max_sockfd) {
    ...
    for (i = 0; i < count; i++) {
        ...
        // 连接peer
        ret = connect(
            peer_sock[i], (struct sockaddr *)&peer_addr[i],
            sizeof(struct sockaddr));
...
/**
 * @brief 解析第一种Tracker的回应消息.
 *        关键字“5:peers”之后是一个B编码的字符串，该字符串以6个字节为一组，
 *        前面4个存放一个peer的IP地址，后面两个存放该peer的端口号
 * 
 * @param buffer 
 * @param ret 
 * @param redirection 
...
int parse_tracker_response1(char *buffer, int ret, char *redirection, int len) {
    ...
    for (; count != 0; count--) {
        ...
        // 将当前结点添加到链表中
        if (p == NULL) {
            if (peer_addr_head == NULL) peer_addr_head = node;
            else {
                p = peer_addr_head;
...
/**
 * @brief 解析第二种Tracker的回应消息。
 *        关键字“5:peers”之后为一个B编码的列表。
 *        列表中每个元素的类型都是字典，一个字典用于表示一个peer。
 * 
 * @param buffer 
 * @param ret 
 * @return int 
...
/**
 * @brief 为已建立连接的peer创建peer结点并加入到peer链表中
 * 
 * @param sock 连接的socket描述符指针
 * @param saptr socket连接的地址结构
 * @return int 失败返回-1，成功返回0
 */
