...
extern Bitmap *bitmap;         // 本地位图
extern int download_piece_num; // 已下载piece个数
extern Peer *peer_head;        // peer链表
...
int last_piece_count = 0; // 最后一个piece下载了多少slice
...
// 是否进入了终端模式：客户端不继续共享已下载的文件，而只是充当下载者。
int end_mode = 0;

/**
 * @brief 创建Btcache结点，分配内存空间并对其成员的值进行初始化
 *
 * @return Btcache* tcache结点指针
 */
...
/**
 * @brief 创建总大小为16K*1024bit即16MB的缓冲区
 *
...
/**
 * @brief
 * 计算一个Btcache结点(即一个slice)中的数据要写到哪个文件的哪个位置，并写入
 *
 * @param node 待写入节点指针
        ...
        if ((line_position < p->length)
            ...
            while (left > 0) {
                if (p->length >= left) { // 当前文件的长度大于等于要写的字节数
                    ...
                    write(
                        fds[i], node->buff + offset,
                        left); // 写入剩余要写的字节数
                    left = 0;
                } else { // 当前文件的长度小于要写的字节数
                    lseek(fds[i], 0, SEEK_SET);
                    write(
                        fds[i], node->buff + offset, p->length); // 写满当前文件
...
/**
 * @brief 从硬盘读出数据，存放到缓冲区中，在peer需要时发送给peer
 *
 * @param node 待读取的 Btcache节点指针
 * @return int 读取成功返回0，读取失败返回-1
 */
...
/**
 * @brief 在peer队列中删除对某个piece的请求
 *
 * @param index piece的index
...
//
/**
 * @brief 检查一个piece的数据是否正确，若正确存入硬盘
 *
 * @param sequnce
 * 存放piece的第一个slice的Btcache结点编号，该值的范围为0～1023中16的整数倍
 * @param peer piece所在的peer节点指针
 * @return int
 */
int write_piece_to_harddisk(int sequnce, Peer *peer) {
    ...
    while (slice_count > 0 && node_ptr != NULL) {
        SHA1Update(&ctx, (unsigned char *)node_ptr->buff, 16 * 1024);
        slice_count--;
        node_ptr = node_ptr->next;
    }
    ...
    printf(
        "writed piece index:%d  total pieces:%d\n", index_copy,
        pieces_length / 20);
...
// 下载完一个slice后, 检查是否该slice为一个piece最后一个slice
// 若是则写入硬盘
// 只在刚刚开始下载时起作用, 以便立即使 peer 得知该 piece 下载完毕
...
int write_last_piece_to_btcache(Peer *peer) {
    ...
    SHA1Init(&ctx);
    while (p != NULL) {
        SHA1Update(&ctx, (unsigned char *)p->buff, p->length);
        p = p->next;
    }
    SHA1Final(piece_hash1, &ctx);
