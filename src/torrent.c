...
//    18*1024即18K是接收缓冲区的大小
//    1500是以太网等局域网一个数据包的最大长度
...
int response_len = 0;          // Tracker回应的总长度
int response_index = 0;        // Tracker回应当前长度
char *tracker_response = NULL; // Tracker回应

int *peer_sock = NULL; // 连接peer的套接字
struct sockaddr_in *peer_addr = NULL;
int *peer_valid = NULL; // 指示连接peer的状态
int peer_count = 0;     // 尝试与多少个peer建立连接

/**
 * @brief 与所有Peer收发数据、交换消息
 *
 * @return int
 */
    ...
    for (;;) {
        ...
        while (p != NULL) {
            if (p->state != CLOSING && FD_ISSET(p->socket, &rset)) {
                ret = recv(
                    p->socket, p->in_buff + p->buff_len, MSG_SIZE - p->buff_len,
                    0);
                if (ret <= 0) { // recv返回0说明对方关闭连接,返回负数说明出错
                    ...
                    p->buff_len += ret;
                    completed = is_complete_message(
                        (unsigned char *)p->in_buff, p->buff_len, &ok_len);
            ...
            for (i = 0; i < tracker_count; i++) {
                if (valid[i] == -1) {
                    ...
                    if (FD_ISSET(sock[i], &wset)) {
                        ...
                        len = sizeof(error);
                        if (getsockopt(
                                sock[i], SOL_SOCKET, SO_ERROR, &error,
                                (socklen_t *)&len)
                            < 0) {
                            valid[i] = 0;
                            close(sock[i]);
                ...
                if (valid[i] == 1 && FD_ISSET(sock[i], &wset)) {
                    ...
                    unsigned long left;
                    left = (pieces_length / 20 - download_piece_num)
                           * piece_length;
                    ...
                    create_request(
                        request, 1024, anouce, listen_port, down, up, left,
                        200);
                ...
                if (valid[i] == 2 && FD_ISSET(sock[i], &rset)) {
                    ...
                    if (ret > 0) {
                        if (response_len != 0) {
                            ...
                            get_response_type(buffer, ret, &response_len)
                            == 1) {
        ...
        if (connecting_peer == 1) {
            for (i = 0; i < peer_count; i++) {
                if (peer_valid[i] == -1 && FD_ISSET(peer_sock[i], &wset)) {
                    ...
                    len = sizeof(error);
                    if (getsockopt(
                            peer_sock[i], SOL_SOCKET, SO_ERROR, &error,
                            (socklen_t *)&len)
                        < 0) {
                        peer_valid[i] = 0;
                    }
...
void print_process_info() {
    ...
    percent = (float)download_piece_num / ((float)pieces_length / 20) * 100;
    if (down_rate >= 1024) down_rate /= 1024;
    if (up_rate >= 1024) up_rate /= 1024;

    if (total_down_rate >= 1024 && total_up_rate >= 1024)
        sprintf(
            info, "Complete:%.2f%% Peers:%d Down:%.2fKB/s Up:%.2fKB/s \n",
            percent, total_peers, down_rate, up_rate);
    else if (total_down_rate >= 1024 && total_up_rate < 1024)
        sprintf(
            info, "Complete:%.2f%% Peers:%d Down:%.2fKB/s Up:%.2fB/s \n",
            percent, total_peers, down_rate, up_rate);
    else if (total_down_rate < 1024 && total_up_rate >= 1024)
        sprintf(
            info, "Complete:%.2f%% Peers:%d Down:%.2fB/s Up:%.2fKB/s \n",
            percent, total_peers, down_rate, up_rate);
    else if (total_down_rate < 1024 && total_up_rate < 1024)
        sprintf(
            info, "Complete:%.2f%% Peers:%d Down:%.2fB/s Up:%.2fB/s \n",
            percent, total_peers, down_rate, up_rate);
